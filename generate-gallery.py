...
# One folder name per line, written by organize-gallery.py.  Folders missing
# from it follow the listed ones in natural name order.
ORDER_FILE = GALLERY / "order.txt"
OPT_DIRNAME = "opt"
...
def read_order() -> list[str]:
    if not ORDER_FILE.is_file():
        return []
    lines = ORDER_FILE.read_text(encoding="utf-8-sig").splitlines()
    return [line.strip() for line in lines if line.strip() and not line.startswith("#")]


def ordered_folders() -> list[Path]:
    rank = {name: index for index, name in enumerate(read_order())}
    folders = sorted((path for path in GALLERY.glob("g*") if path.is_dir()), key=natural_key)
    return sorted(folders, key=lambda path: rank.get(path.name, len(rank)))
